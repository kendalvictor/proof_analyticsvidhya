...
set.seed(1234)
modelo10 <- randomForest( Loan_Status~.,data = train.data,   # Datos a entrenar 
...
##probabilidades
proba10<-predict(modelo10, newdata=test.data,type="prob")
proba10=proba10[,2]

# curva ROC
AUC10 <- roc(test.data$Loan_Status, proba10) 
...
# Calcular los valores predichos
PRED <-predict(modelo10,test.data,type="class")
...
# Calcular el error de mala clasificaci?n
error10=mean(PRED!=test.data$Loan_Status)
